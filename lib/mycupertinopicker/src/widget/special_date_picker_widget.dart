...
/// @author dylan wu
/// @since 2019-05-10
class SpecialDatePickerWidget extends StatefulWidget {
  SpecialDatePickerWidget({
    Key key,
    this.minDateTime,
    this.maxDateTime,
  ...
  @override
  State<StatefulWidget> createState() => _SpecialDatePickerWidgetState(
      this.minDateTime, this.maxDateTime, this.initialDateTime);
}

class _SpecialDatePickerWidgetState extends State<SpecialDatePickerWidget> {
  DateTime _minDateTime, _maxDateTime;
  int _currYear, _currMonth, _currDay;
  List<int> _yearRange, _dayRange;
  List<Object> _monthRange;
  ...
  Map<String, List<Object>> _valueRangeMap;

  String noexp = "Pas D'Expérience";

  bool _isChangeDateRange = false;

  _SpecialDatePickerWidgetState(
  ...
  Widget _renderPickerView(BuildContext context) {
    Widget specialDatePickerWidget = _renderSpecialDatePickerWidget();
    ...
    if (widget.pickerTheme.title != null || widget.pickerTheme.showTitle) {
      ...
      return Column(children: <Widget>[titleWidget, specialDatePickerWidget]);
    }
    return specialDatePickerWidget;
  }
  ...
  Widget _renderSpecialDatePickerWidget() {
      ...
      child: Container(
        ...
        child: CupertinoPicker.builder(
          ...
          itemBuilder: (context, index) =>
              index <= valueRange.last-1 ? 
                _renderDatePickerItemComponent(valueRange.first + index - 1, format)
                : _renderSpecialDatePickerItemComponent(noexp),
        ),
      ),
    );
  }

  Widget _renderSpecialDatePickerItemComponent(String value) {
    return Container(
  ...
  void _changeYearSelection(int index) {
    print("voici l'index: " + index.toString());
    if(index < (_maxDateTime.year - _minDateTime.year)) {
  ...
  void _changeMonthSelection(int index) {
    var first = _monthRange.first as num;
    int month = first + index;
    if (_currMonth != month) {
      _currMonth = month;
      _changeDateRange();
      _onSelectedChange();
